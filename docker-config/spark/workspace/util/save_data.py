...
def save_data_to_minio(
    ...
    try:
        df.write.mode(mode).format(fmat).partitionBy(partitionField).save(path=path)
...
def save_to_database(
    ...
    try:
        df.write.format(fmat).mode(mode).option("url", url).option(
            "driver", driver
        ).option("dbtable", table).option("user", username).option(
            "password", password
        ).save()
    except Exception as e:
        print(f"Error: {e}")
