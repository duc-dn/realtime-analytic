...
class DeltaSink:
    ...
    def merge_data_to_delta_table(df, delta_table, pk, pk1=None, pk2=None):
        if pk1 and pk2:
            primary_keys_fulfill1 = [
                when(
                    col("parsed_value.op") == "d", 
                    col(f"parsed_value.before.{pk1}")
                )
                .otherwise(
                    col(f"parsed_value.after.{pk1}")
                )
                .alias(f"before_{pk1}")
            ]
            ...
            primary_keys_fulfill2 = [
                when(
                    col("parsed_value.op") == "d", 
                    col(f"parsed_value.before.{pk2}")
                )
                .otherwise(
                    col(f"parsed_value.after.{pk2}")
                )
                .alias(f"before_{pk2}")
            ]
            ...
            primary_keys_fulfill = [
                when(
                    col("parsed_value.op") == "d", 
                    col(f"parsed_value.before.{pk}")
                )
                .otherwise(
                    col(f"parsed_value.after.{pk}")
                )
                .alias(f"before_{pk}")
            ]
