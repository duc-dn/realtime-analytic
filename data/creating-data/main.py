...
phone_numbers_provider = DynamicProvider(
    ...
    elements=pd.read_csv(f"{os.getcwd()}/data_csv/phone_numbers.csv")[
        "phone_number"
    ].tolist(),
)
...
def gen_user(num=1):
    ...
    for i in tqdm(range(1, num)):
        ...
        user["created_at"] = (
            int(local_time.timestamp()) - random.randint(0, 100) * time_days
        )
...
def gen_brand(num=1):
    brand_name = pd.read_csv((f"{os.getcwd()}/data_csv/brand_names.csv"))[
        "brand_name"
    ].to_list()
...
def gen_products():
    ...
    df["category_id"] = np.select(
        category_filter, choices_categories, default=-10000000000
    )
    ...
    df["description"] = "designed by " + df["products_name"].str.extract(
        r"(\w+)", expand=False
    )
...
def generate_created_at():
    return int(local_time.timestamp()) - random.randint(0, 100) * time_days
